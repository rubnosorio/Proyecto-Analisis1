    ...
    app.post('/nota_estudiantes', (req, res) => {
        ...
        USUARIO.id_usuario=USUARIO_CLASE.id_usuario`, function (err, rows, fields) {
            ...
            if (!err) {
                ...
                connection.query(`select distinct CLASE.nombre_clase, EXAMEN.id_examen, EXAMEN.nombre_examen, ENTREGA_EXAMEN.nota_examen, USUARIO.id_usuario, USUARIO.nombres, USUARIO.apellidos
                from CLASE, EXAMEN, ENTREGA_EXAMEN, USUARIO_CLASE, USUARIO
                where EXAMEN.id_clase=CLASE.id_clase and
                CLASE.id_clase=${parseInt(id_clase)} and
                EXAMEN.id_examen=ENTREGA_EXAMEN.id_examen and
                USUARIO_CLASE.id_usuario= ENTREGA_EXAMEN.id_usuario and
                USUARIO.id_usuario=USUARIO_CLASE.id_usuario`, function (err2, rows2, fields) {
                    if (!err2) {
                        for (let i = 0; i < lista_notas.length; i++) {
                            //buscamos las notas de examenes dentro del resultado
                            for (let j = 0; j < rows2.length; j++) {
                                if (rows2[j].id_usuario == lista_notas[i].id_estudiante) {
                                    var examen = {
                                        id_examen: rows2[j].id_examen,
                                        examen: rows2[j].nombre_examen,
                                        nota: rows2[j].nota_examen
                                    }
                                    lista_notas[i].lista_examenes.push(examen);
                                }
                            }
                        }
